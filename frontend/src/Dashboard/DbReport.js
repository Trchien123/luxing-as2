...
const ReportTable = () => {
  ...
  const handleToggleError = () => {
    if (hasError) {
      ...
      setHasError(true);
      setErrorMessage("Error log: error here");
    }
  };
  ...
  <div className="icon-and-log">
        <img
          src={hasError ? "/icon/error.png" : "/icon/approve.png"}
          alt={hasError ? "Error icon" : "Approval icon"}
          className="status-icon"
        />
        {hasError && <div className="error-log">{errorMessage}</div>}
      </div>
      { /*for checking different interfaces*/}
